...
function Aside() {
  ...
  return (
    ...
    <aside className='p-4 pr-0 h-full flex'>
      <div className='pt-16 pb-3 px-3 gap-8 h-full flex flex-col relative border bg-background-2 cursor-pointer rounded-xl'>
    ...
    // <aside className='pt-16 pb-3 px-3 gap-8 h-full flex flex-col relative border bg-background-2 border-t-0'>
    ...
    //   <section onClick={() => push('configuracoes')} className='button relative gap-3 flex justify-start items-center cursor-pointer'>
    //     <Svg.Gear className={`w-6 h-6 ${ page == 'configuracoes' ? 'fill-primary' : 'fill-color-1'}`}/>
    //     <Description className={`mt-1 ${ page == 'configuracoes' ? 'text-primary' : ''}`}>Configurações</Description>
    //     <span className={`absolute -right-4 w-[.3rem] h-[160%] bg-primary rounded-lg ${page == 'configuracoes' ? '' : 'hidden'}`}></span>
    //   </section>

    //   <section className='mt-auto gap-2 flex justify-center items-center'>
    //     <Label className='mt-1'>versão <b className='text-primary'>{Package.version}</b></Label>
    //   </section>
